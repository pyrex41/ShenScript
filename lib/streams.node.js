...
export class NodeInStream {
  constructor(stream, name) {
    ...
    this.buf = '';
    this.pos = 0;
    // Set once the underlying stream has reported done (EOF). The kernel REPL
    // (shen.loop) tail-calls itself forever; on a closed stdin lineread raises
    // "error: empty stream" every iteration, which the loop traps and ignores,
    // spinning indefinitely. The JS-level repl in bin/shen.js polls this flag
    // to break the loop and exit cleanly, mirroring shen-go's StinputEOF.
    this.eof = false;
  }
  async read() {
    ...
    return done ? (this.eof = true, -1) : (this.buf = value, this.pos = 0, this.read());
  }
  close() {
...
// immediately read it back (e.g. Yggdrasil's bootstrap) depend on -
