...
const escapeCharacter = ch => validCharacterRegex.test(ch) ? ch : `$${hex(ch)}`;
...
const avoidReserved = x => x && reserved.has(x) ? '$_' + x : x;

const statementTypes = new Set([
  'BlockStatement',  'DebuggerStatement',   'DoWhileStatement', 'EmptyStatement',  'ExpressionStatement',
  'ForStatement',    'ForInStatement',      'ForOfStatement',   'IfStatement',     'ReturnStatement',
  'TryStatement',    'VariableDeclaration', 'WhileStatement'
]);
const isStatement = x => x && statementTypes.has(x.type);
...
const Block = (...body) => ({ type: 'BlockStatement', body: body.map(Statement) });
const Call = (callee, args, async = false) => {
  const ast = { type: 'CallExpression', callee, arguments: args };
  return async ? Await(ast) : ast;
};
...
const DoWhile = (test, body) => ({ type: 'DoWhileStatement', test, body });
const Empty = () => ({ type: 'EmptyStatement' });
const For = (init, test, update, body) => ({ type: 'ForStatement', init, test, update, body });
const ForIn = (left, right, body) => ({ type: 'ForInStatement', left, right, body });
const ForOf = (left, right, body, awaited = false) => ({ type: 'ForOfStatement', left, right, body, await: awaited });
...
const Statement = expression => isStatement(expression) ? expression : ({ type: 'ExpressionStatement', expression });
