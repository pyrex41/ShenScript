export default $ => {
  const {
    asArray, asCons, asNumber, asOutStream, asShenBool, asString, defun, equate,
  ...
  // lambda forms, so provide the documented property-vector adapter.
  defun('shen.set-lambda-form-entry', entry =>
    isCons(entry)
      ? (lookup('put').f(entry.head, s`shen.lambda-form`, entry.tail, valueOf('*property-vector*')), entry)
