import { AsyncFunction, GeneratorFunction, flatMap, last, mapAsync, pairs } from './utils.js';
...
export default async $ => {
  const {
    asFunction, asJsBool, asList, asNumber, asShenBool, asString, asSymbol, assign, compile,
    defun, evalJs, fun, inline, isArray, lookup, nameOf, s, settle, symbolOf,
    toArray, toArrayTree, toList, toListTree, valueOf
  } = $;
  const propertyVectorCell = lookup('*property-vector*');
  const caller = name => {
    const cell = lookup(name);
    return (...args) => settle(cell.f(...args));
  };
  const curryType = caller('shen.curry-type');
  const declare = caller('declare');
  const recordMacro = caller('shen.record-macro');
  const load = caller('load');
  const parse = caller('read-from-string');
  const put = caller('put');
  const evaluate = caller('eval');
  const define = async (name, f) => {
  ...
  const declareType = async (name, type) => await declare(symbolOf(name), type);
  const defineTyped = async (name, type, f) => {
  ...
  const defmacro = async (name, f) => {
    ...
    const id = symbolOf(name);
    await recordMacro(id, fun(x => macrofn(x)));
    return id;
  };
  const evalShen = expr => evaluate(toListTree(expr));
  const execEach = async source => mapAsync(toArray(await parse(source)), evalShen);
  const exec = async source => last(await execEach(source));
  const symbol = async (name, value) => {
    const index = name.lastIndexOf('.') + 1;
    const starredName = `${name.substr(0, index)}*${name.substr(index)}*`;
    assign(starredName, value);
    await define(name, () => valueOf(starredName));
    return value;
  };
  const sleep = duration => new Promise(resolve => setTimeout(() => resolve(null), duration));
  const delay = (duration, callback) => setTimeout(() => settle(callback()), duration);
  const repeat = (duration, callback) => setInterval(() => settle(callback()), duration);
  ...
  inline('js.raw.==',          null, [null, null], (x, y) => Binary('==', x, y));
  inline('js.raw.===',         null, [null, null], (x, y) => Binary('===', x, y));
  inline('js.raw.!=',          null, [null, null], (x, y) => Binary('!=', x, y));
  inline('js.raw.!==',         null, [null, null], (x, y) => Binary('!==', x, y));
  inline('js.raw.not',         null, [null],            x => Unary('!', x));
  inline('js.raw.and',         null, [null, null], (x, y) => Binary('&&', x, y));
  inline('js.raw.or',          null, [null, null], (x, y) => Binary('||', x, y));
  inline('js.raw.+',           null, [null, null], (x, y) => Binary('+', x, y));
  inline('js.raw.-',           null, [null, null], (x, y) => Binary('-', x, y));
  inline('js.raw.*',           null, [null, null], (x, y) => Binary('*', x, y));
  inline('js.raw./',           null, [null, null], (x, y) => Binary('/', x, y));
  ...
  inline('js.raw.<=',          null, [null, null], (x, y) => Binary('<=', x, y));
  inline('js.raw.>=',          null, [null, null], (x, y) => Binary('>=', x, y));
  inline('js.raw.bitwise.not', null, [null],            x => Unary('~', x));
  inline('js.raw.bitwise.and', null, [null, null], (x, y) => Binary('&', x, y));
  inline('js.raw.bitwise.or',  null, [null, null], (x, y) => Binary('|', x, y));
  inline('js.raw.bitwise.xor', null, [null, null], (x, y) => Binary('^', x, y));
  inline('js.raw.<<',          null, [null, null], (x, y) => Binary('<<', x, y));
  inline('js.raw.>>',          null, [null, null], (x, y) => Binary('>>', x, y));
  inline('js.raw.>>>',         null, [null, null], (x, y) => Binary('>>>', x, y));
  inline('js.raw.delete',      null, [null, null], (x, y) => Unary('delete', Member(x, y)));
  inline('js.raw.eval',        null, [null],            x => Call(Id('eval'), [x]));
  inline('js.raw.in',          null, [null, null], (x, y) => Binary('in', x, y));
  inline('js.raw.instanceof',  null, [null, null], (x, y) => Binary('instanceof', x, y));
  inline('js.raw.typeof',      null, [null],            x => Unary('typeof', x));
  inline('js.raw.void',        null, [null],            x => Unary('void', x));
  ...
  await define('json.parse', s => JSON.parse(s));
  await define('json.str',   x => JSON.stringify(x));
  ...
  await define('shen-script.lookup', x => lookup(nameOf(asSymbol(x))));
  ...
  await define('shen-script.array->list', x => toList(x));
  await define('shen-script.array->list+', (x, y) => toList(x, y));
  await define('shen-script.array->list-tree', x => toListTree(x));
  await define('shen-script.list->array', x => toArray(x));
  await define('shen-script.list->array-tree', x => toArrayTree(x));

  await define('shen-script.boolean.js->shen', x => asShenBool(x));
  await define('shen-script.boolean.shen->js', x => asJsBool(x));

  return Object.assign($, {
    caller, define, defineTyped, defmacro, evalShen, exec, execEach, load, parse, symbol
  });
};
