...
const primitives = [
  ...
  // shen.read-unit-string is deliberately absent: readString on an
  // incremental stream can return a Promise, so callers must stay async
  s`shen.char-stinput?`, s`shen.char-stoutput?`, s`shen.write-string`,

  // overrides (see lib/overrides.js). put and the dict layer are no longer
  // overridden: the S41.2 refresh dropped dict.kl and put now hashes into the
  // vector-based property store defined in sys.kl.
  s`shen.pvar?`, s`@p`, s`shen.byte->digit`, s`integer?`, s`symbol?`,
  s`variable?`, s`shen.fillvector`
];
...
const buildValue = (context, [_value, symbol]) =>
  isSymbol(symbol) && !context.has(symbol)
    ...
    : assemble(
        s => Call$('valueOf', [s]),
        inlineName(context, symbol));
// inliners run with the build context as `this` so an inliner whose
// expansion is only valid in async contexts (read-byte below) can adapt;
// most inliners are arrows and simply ignore it
const buildInline = (context, [fExpr, ...argExprs]) =>
  assemble(
    (...xs) => context.inlines.get(nameOf(fExpr)).apply(context, xs),
...
export default (options = {}) => {
  ...
  const inline = (name, dataType, paramTypes, f) => {
    const inliner = function (...args) {
      const ast = f.apply(this, args.map((a, i) => paramTypes[i] ? cast(paramTypes[i], a) : uncast(a)));
  ...
  // read() on an incremental stream (stdin) returns a Promise, and the byte
  // flows straight into comparisons and trampoline arguments, so it must be
  // settled here: maybe-await keeps sync streams (files, test fixtures) on
  // the no-await fast path while making async streams correct. The await
  // also keeps any function reading bytes async (structural demotion in
  // buildFunction sees it), which is required for the suspension to land.
  inline('read-byte',       'Number', ['InStream'],                function (x) {
    const read = Call(Member(x, Id('read')), []);
    return this.async ? MaybeAwait(read) : read;
  });
