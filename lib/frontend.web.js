...
export default async $ => {
  ...
  await define('local-storage.clear', () => (localStorage.clear(), null));
  await define('local-storage.get', key => localStorage.getItem(key));
  await define('local-storage.has?', key => asShenBool(localStorage.getItem(key) !== undefined));
  await define('local-storage.remove', key => (localStorage.removeItem(key), null));
  await define('local-storage.set', (key, value) => (localStorage.setItem(key, value), null));
