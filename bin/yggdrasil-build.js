#!/usr/bin/env node
// Yggdrasil stage-2 builder for ShenScript.
//
//   node bin/yggdrasil-build.js <shaken-dir> <out.js> [--linked | --web]
//
...
//     host I/O).
//
// Builder contract (yggdrasil.shen): load kernel.kl's defuns, call the
...
const USAGE = 'usage: node bin/yggdrasil-build.js <shaken-dir> <out.js> [--linked | --web]';
...
if (!fs.existsSync(manifestPath)) {
  console.error(`no yggdrasil.manifest.txt in ${shakenDir}`);
  process.exit(1);
}
...
if (manifest['manifest-version'] !== '3') {
  console.warn(`warning: manifest-version is ${manifest['manifest-version']}, this builder understands version 3`);
}
...
const header = `// Generated by ShenScript's Yggdrasil stage-2 builder.
